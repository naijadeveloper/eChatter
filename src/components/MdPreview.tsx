  ...
  return (
    <>
      <div className="dropdown-scroll relative h-full w-full  overflow-y-auto overflow-x-hidden rounded rounded-tr-none border-2 border-transparent bg-gray-300 dark:bg-gray-800">
        ...
        {title && (
          <p className={`mx-auto ${image? "mt-3" : "mt-12"} flex w-[96%] items-center justify-center text-center text-4xl font-bold capitalize underline decoration-gray-500 underline-offset-2`}>
            {title}
          </p>
        )}
        ...
        <ReactMarkdown
          children={content}
          className={`mdstyleAll prose prose-gray ${(image || title || tags.length > 0)? "mt-2" : "mt-10"} min-h-full min-w-full max-w-full rounded p-2 dark:prose-invert md:prose-lg prose-headings:break-words prose-headings:mb-2 prose-headings:mt-5 prose-p:break-words prose-p:my-2 prose-a:text-blue-600 prose-pre:border-2 prose-pre:border-transparent prose-pre:bg-gray-900 dark:prose-a:text-blue-400`}
        />
